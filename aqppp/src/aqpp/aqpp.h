...
namespace aqppp {
	...
	public:
		Aqpp(int sample_row_num, double sample_rate, double ci_index);
		...
		std::pair<double, double> AqppSumQuery(int query_id, FILE* info_file, const std::vector<std::vector<double>> &sample, const std::vector<std::vector<double>> &small_sample, const std::vector<std::vector<CA>>& mtl_points, const MTL_STRU& mtl_res, const std::vector<Condition>& user_demands);
		...
		*/
		...
		double ComputeDifference4Sum(const std::vector<std::vector<double>>&sample, const std::vector<Condition>& user_demands, const std::vector<Condition>& mtl_choices);
		...
		void FinalComputeDifference(const std::vector<std::vector<double>>&sample, const std::vector<Condition>& user_demands, const std::vector<Condition>& mtl_choices, std::vector<std::pair<double, double>>& final_diff_res);
		...
		*/
		void FillMaterializeChoices(const int col, const std::vector<std::vector<double>>& small_sample, const std::vector<RGs>& all_rgs, const std::vector<Condition>& user_demands, double &o_min_ci, std::vector<Condition >& mtl_choices, std::vector<Condition>& final_mtl_choices);
	};
}
